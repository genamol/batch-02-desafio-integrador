...
async function main() {
  ...
  var UpgradeableToken = await ethers.getContractFactory("BBitesTokenUpgradeable");
...
async function upgrade() {
  const ProxyAddress = " ";
  const BBitesTokenUpgradeableV2 = await ethers.getContractFactory(
    "BBitesTokenUpgradeable"
  );
  const bBitesTokenUpgradeableV2 = await upgrades.upgradeProxy(
    ProxyAddress,
    BBitesTokenUpgradeableV2
  );
